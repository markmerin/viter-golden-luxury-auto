...
const NadaDepreciationWithAddList = ({ setItemEdit }) => {
  ...
  const [carMakeStatus, setCarMakeStatus] = React.useState("all");
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/nada-depreciation-with-add/search`, // search endpoint
        `${apiVersion}/nada-depreciation-with-add/page/${pageParam}/${carId}`, // list endpoint
        store.isSearch, // search boolean
        {
          searchValue: search?.current?.value,
          nada_depreciation_with_add_car_id: carId,
          isFilter,
          nada_depreciation_with_add_is_active: carMakeStatus,
        }
      ),
  ...
  const handleArchive = (item) => {
    ...
    setId(item.nada_depreciation_with_add_aid);
  };
  ...
  const handleChangeCarMakeStatus = (e) => {
    setCarMakeStatus(e.target.value);
    setIsFilter(false);
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2">
            <div className="relative w-28 ">
              ...
              <select
                name="status"
                value={carMakeStatus}
                onChange={(e) => handleChangeCarMakeStatus(e)}
                    ...
                    return (
                      <tr key={key} className="relative group">
                        ...
                        <td className="pl-3 sm:hidden">
                          {item.nada_depreciation_with_add_is_active === 1 ? (
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/nada-depreciation-with-add/active/${id}`}
      ...
      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/nada-depreciation-with-add/active/${id}`}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/nada-depreciation-with-add/${id}`}
          msg={"Are you sure you want to delete this record?"}
          successMsg={"Deleted successfully."}
          item={dataItem.nada_depreciation_with_add_name}
          queryKey={"nada-depreciation-with-add"}
        />
      )}
    </>
...
export default NadaDepreciationWithAddList;
