...
const ModalAddCurrentCost = ({ itemEdit }) => {
  const { dispatch } = React.useContext(StoreContext);
  const [show, setShow] = React.useState("show");
  const [animate, setAnimate] = React.useState("translate-x-full");

  const initVal = {
    current_cost_aid: itemEdit ? itemEdit.current_cost_aid : "",
    current_cost_name: itemEdit ? itemEdit.current_cost_name : "",
    current_cost_name_old: itemEdit ? itemEdit.current_cost_name : "",
  };

  const yupSchema = Yup.object({
    current_cost_name: Yup.string().required("Required"),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/current-cost/${itemEdit.current_cost_aid}`
          : `${apiVersion}/current-cost`,
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["current-cost"] });
...
export default ModalAddCurrentCost;
