...
const ModalAddHistory = ({ itemEdit }) => {
  const { dispatch } = React.useContext(StoreContext);
  const [show, setShow] = React.useState("show");
  const [animate, setAnimate] = React.useState("translate-x-full");

  const initVal = {
    history_aid: itemEdit ? itemEdit.history_aid : "",
    history_name: itemEdit ? itemEdit.history_name : "",
    history_name_old: itemEdit ? itemEdit.history_name : "",
  };

  const yupSchema = Yup.object({
    history_name: Yup.string().required("Required"),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/history/${itemEdit.history_aid}`
          : `${apiVersion}/history`,
        itemEdit ? "put" : "post",
        values
      ),
