...
const PurchaseFinancedList = ({ setItemEdit }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const [id, setId] = React.useState(null);
  const [dataItem, setData] = React.useState(null);
  const [onSearch, setOnSearch] = React.useState(false);
  const [isFilter, setIsFilter] = React.useState(false);
  const [purchaseFinancedStatus, setPurchaseFinancedStatus] =
    React.useState("all");
  ...
  const {
    ...
    queryKey: [
      ...
      store.isSearch,
      purchaseFinancedStatus,
    ],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/purchase-financed/search`, // search endpoint
        `${apiVersion}/purchase-financed/page/${pageParam}`, // list endpoint
        store.isSearch, // search boolean
        {
          searchValue: search?.current?.value,
          id: "",
          isFilter,
          purchase_financed_is_active: purchaseFinancedStatus,
        }
      ),
  ...
  const handleArchive = (item) => {
    ...
    setId(item.purchase_financed_aid);
  };
  ...
  const handleChangePurchaseFinancedStatus = (e) => {
    setPurchaseFinancedStatus(e.target.value);
    setIsFilter(false);
  ...
  return (
    <>
        ...
        <div className="md:flex grid grid-cols-[1fr_3.1rem] items-center gap-2 w-full xl:w-1/2">
          <div className="flex items-center gap-2">
            <div className="relative w-28 ">
              ...
              <select
                name="status"
                value={purchaseFinancedStatus}
                onChange={(e) => handleChangePurchaseFinancedStatus(e)}
                    ...
                    return (
                      <tr key={key} className="relative group">
                        ...
                        <td className="pl-3 sm:hidden">
                          {item.purchase_financed_is_active === 1 ? (
                        ...
                        <td>{item.purchase_financed_name}</td>
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/purchase-financed/active/${id}`}
          msg={"Are you sure you want to archive this record?"}
          successMsg={"Archived successfully."}
          queryKey={"purchasefinanced"}
        />
      )}

      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/purchase-financed/active/${id}`}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/purchase-financed/${id}`}
          msg={"Are you sure you want to delete this record?"}
          successMsg={"Deleted successfully."}
          item={dataItem.purchase_financed_name}
          queryKey={"purchasefinanced"}
        />
      )}
    </>
...
export default PurchaseFinancedList;
