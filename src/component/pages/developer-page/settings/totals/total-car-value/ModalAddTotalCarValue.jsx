...
const ModalAddTotalCarValue = ({ itemEdit }) => {
  const { dispatch } = React.useContext(StoreContext);
  const [show, setShow] = React.useState("show");
  const [animate, setAnimate] = React.useState("translate-x-full");

  const initVal = {
    car_value_aid: itemEdit ? itemEdit.car_value_aid : "",
    car_value_name: itemEdit ? itemEdit.car_value_name : "",
    car_value_name_old: itemEdit ? itemEdit.car_value_name : "",
  };

  const yupSchema = Yup.object({
    car_value_name: Yup.string().required("Required"),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/total-car-value/${itemEdit.car_value_aid}`
          : `${apiVersion}/total-car-value`,
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["totalcarvalue"] });
...
export default ModalAddTotalCarValue;
