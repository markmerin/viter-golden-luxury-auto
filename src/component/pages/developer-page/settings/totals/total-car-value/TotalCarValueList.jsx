...
const TotalCarValueList = ({ setItemEdit }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const [id, setId] = React.useState(null);
  const [dataItem, setData] = React.useState(null);
  const [onSearch, setOnSearch] = React.useState(false);
  const [isFilter, setIsFilter] = React.useState(false);
  const [carValueStatus, setCarValueStatus] = React.useState("all");
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/total-car-value/search`, // search endpoint
        `${apiVersion}/total-car-value/page/${pageParam}`, // list endpoint
        store.isSearch, // search boolean
        {
          searchValue: search?.current?.value,
          id: "",
          isFilter,
          car_value_is_active: carValueStatus,
        }
      ),
  ...
  const handleChangeCarStatus = (e) => {
    setCarValueStatus(e.target.value);
    setIsFilter(false);
  ...
  return (
    <>
        ...
        <div className="md:flex grid grid-cols-[1fr_3.1rem] items-center gap-2 w-full xl:w-1/2">
          <div className="flex items-center gap-2">
            <div className="relative w-28 ">
              ...
              <select
                name="status"
                value={carValueStatus}
                onChange={(e) => handleChangeCarStatus(e)}
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/total-car-value/active/${id}`}
          msg={"Are you sure you want to archive this record?"}
          successMsg={"Archived successfully."}
          queryKey={"totalcarvalue"}
        />
      )}

      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/total-car-value/active/${id}`}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/total-car-value/${id}`}
          msg={"Are you sure you want to delete this record?"}
          successMsg={"Deleted successfully."}
          item={dataItem.car_value_name}
          queryKey={"totalcarvalue"}
        />
      )}
    </>
...
export default TotalCarValueList;
