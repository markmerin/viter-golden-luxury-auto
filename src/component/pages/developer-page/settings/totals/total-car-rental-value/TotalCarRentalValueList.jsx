...
const TotalCarRentalValueList = ({ setItemEdit }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const [id, setId] = React.useState(null);
  const [dataItem, setData] = React.useState(null);
  const [onSearch, setOnSearch] = React.useState(false);
  const [isFilter, setIsFilter] = React.useState(false);
  const [carRentalStatus, setCarRentalStatus] = React.useState("all");
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/total-car-rental-value/search`, // search endpoint
        `${apiVersion}/total-car-rental-value/page/${pageParam}`, // list endpoint
        store.isSearch, // search boolean
        {
          searchValue: search?.current?.value,
          id: "",
          isFilter,
          car_rental_value_is_active: carRentalStatus,
        }
      ),
  ...
  const handleArchive = (item) => {
    ...
    setId(item.car_rental_value_aid);
  };
  ...
  const handleChangeCarRentalStatus = (e) => {
    setCarRentalStatus(e.target.value);
    setIsFilter(false);
  ...
  return (
    <>
        ...
        <div className="md:flex grid grid-cols-[1fr_3.1rem] items-center gap-2 w-full xl:w-1/2">
          <div className="flex items-center gap-2">
            <div className="relative w-28 ">
              ...
              <select
                name="status"
                value={carRentalStatus}
                onChange={(e) => handleChangeCarRentalStatus(e)}
                    ...
                    return (
                      <tr key={key} className="relative group">
                        ...
                        <td className="pl-3 sm:hidden">
                          {item.car_rental_value_is_active === 1 ? (
                        ...
                        <td>{item.car_rental_value_name}</td>
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/total-car-rental-value/active/${id}`}
          msg={"Are you sure you want to archive this record?"}
          successMsg={"Archived successfully."}
          queryKey={"totalcarrentalvalue"}
        />
      )}

      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/total-car-rental-value/active/${id}`}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/total-car-rental-value/${id}`}
          msg={"Are you sure you want to delete this record?"}
          successMsg={"Deleted successfully."}
          item={dataItem.car_rental_value_name}
          queryKey={"totalcarrentalvalue"}
        />
      )}
    </>
...
export default TotalCarRentalValueList;
