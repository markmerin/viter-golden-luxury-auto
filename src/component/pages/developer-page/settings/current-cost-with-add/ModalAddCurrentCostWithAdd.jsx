...
const ModalAddCurrentCostWithAdd = ({ itemEdit }) => {
  const { dispatch } = React.useContext(StoreContext);
  const [show, setShow] = React.useState("show");
  const [animate, setAnimate] = React.useState("translate-x-full");

  const initVal = {
    current_cost_with_add_aid: itemEdit
      ? itemEdit.current_cost_with_add_aid
      : "",
    current_cost_with_add_name: itemEdit
      ? itemEdit.current_cost_with_add_name
      : "",
    current_cost_with_add_name_old: itemEdit
      ? itemEdit.current_cost_with_add_name
      : "",
  };

  const yupSchema = Yup.object({
    current_cost_with_add_name: Yup.string().required("Required"),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/current-cost-with-add/${itemEdit.current_cost_with_add_aid}`
          : `${apiVersion}/current-cost-with-add`,
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["current-cost-with-add"] });
                  ...
                  <div className="modal-overflow">
                    <div className="relative mt-5 mb-6">
                      <InputText
                        label="Name"
                        type="text"
                        name="current_cost_with_add_name"
                        disabled={mutation.isPending}
                      />
                    </div>
...
export default ModalAddCurrentCostWithAdd;
