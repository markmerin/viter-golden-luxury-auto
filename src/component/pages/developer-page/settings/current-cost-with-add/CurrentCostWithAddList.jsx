...
const CurrentCostWithAddList = ({ setItemEdit }) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/current-cost-with-add/search`, // search endpoint
        `${apiVersion}/current-cost-with-add/page/${pageParam}`, // list endpoint
        store.isSearch, // search boolean
        {
          searchValue: search?.current?.value,
          id: "",
          isFilter,
          current_cost_with_add_is_active: carMakeStatus,
        }
      ),
  ...
  const handleArchive = (item) => {
    ...
    setId(item.current_cost_with_add_aid);
  };
  ...
  return (
    <>
                    ...
                    return (
                      <tr key={key} className="relative group">
                        ...
                        <td className="pl-3 sm:hidden">
                          {item.current_cost_with_add_is_active === 1 ? (
                        ...
                        <td>{item.current_cost_with_add_name}</td>
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/current-cost-with-add/active/${id}`}
          msg={"Are you sure you want to archive this record?"}
          successMsg={"Archived successfully."}
          queryKey={"current-cost-with-add"}
        />
      )}
      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/current-cost-with-add/active/${id}`}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/current-cost-with-add/${id}`}
          msg={"Are you sure you want to delete this record?"}
          successMsg={"Deleted successfully."}
          item={dataItem.current_cost_with_add_name}
          queryKey={"current-cost-with-add"}
        />
      )}
    </>
...
export default CurrentCostWithAddList;
