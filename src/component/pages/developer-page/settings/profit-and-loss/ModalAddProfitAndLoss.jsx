...
const ModalAddProfitAndLoss = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/profit-and-loss/${itemEdit.profit_and_loss_aid}`
          : `${apiVersion}/profit-and-loss`,
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    profit_and_loss_name: itemEdit ? itemEdit.profit_and_loss_name : "",

    profit_and_loss_name_old: itemEdit ? itemEdit.profit_and_loss_name : "",
  };

  const yupSchema = Yup.object({
    profit_and_loss_name: Yup.string().required("Required"),
  });
      ...
      >
        <div className="relative mb-4 modal_header">
          <h3 className="text-sm text-black">
            {itemEdit ? "Update" : "Add"} Profits and Loss
          </h3>
          <button
            type="button"
...
export default ModalAddProfitAndLoss;
