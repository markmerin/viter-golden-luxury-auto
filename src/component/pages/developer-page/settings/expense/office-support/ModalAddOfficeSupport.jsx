...
const ModalAddOfficeSupport = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/office-support/${itemEdit.office_support_aid}`
          : `${apiVersion}/office-support`,
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["officeSupport"] });
  ...
  const initVal = {
    office_support_name: itemEdit ? itemEdit.office_support_name : "",

    office_support_name_old: itemEdit ? itemEdit.office_support_name : "",
  };

  const yupSchema = Yup.object({
    office_support_name: Yup.string().required("Required"),
  });
      ...
      >
        <div className="relative mb-4 modal_header">
          <h3 className="text-sm text-black">
            {itemEdit ? "Update" : "Add"} Office Support
          </h3>
          <button
            type="button"
...
export default ModalAddOfficeSupport;
