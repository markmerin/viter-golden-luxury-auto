...
if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    ...
    checkApiKey();
    checkPayload($data);
    $cost->current_cost_with_add_search = $data["searchValue"];

    // get data
    if ($data["isFilter"] == true) {

        // if filter with search
        if ($cost->current_cost_with_add_search != "") {

            checkKeyword($cost->current_cost_with_add_search);
            $cost->current_cost_with_add_is_active = checkIndex($data, "current_cost_with_add_is_active");
